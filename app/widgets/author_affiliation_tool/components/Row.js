...
define([
  'react',
  '../components/LastActiveDateRow',
  '../components/AffiliationRow'
  ...
  const Row = ({ onChange, data }) => {
    const {
      author, selected, affiliations, lastActiveDates
    } = data;
    ...
    return (
      <div className="row">
        ...
        <div className="col-xs-8">
          {affiliations.map(a => <AffiliationRow
              key={a.id}
              onChange={() => onChange(a)}
              {...a} />
          )}
        ...
        <div className="col-xs-2">
          {lastActiveDates.map(d => <LastActiveDateRow
              key={d.id}
              onChange={() => onChange(d)}
              {...d}
            />
